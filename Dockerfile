FROM ubuntu:groovy
RUN apt-get update -qq && apt-get install make git zlib1g-dev libssl-dev gperf php cmake clang-10 libc++-dev libc++abi-dev \
git clone https://github.com/tdlib/td.git /root/td && mkdir /root/td/build \
